...
def dynamic_programming(items, budget):
    ...
    for i in range(1, num_items + 1):
        for j in range(budget + 1):
            ...
            if current_item['cost'] <= j:
                ...
                with_current = dp_table[i - 1][j - current_item['cost']] + current_item['calories']
                
                if with_current > without_current:
                    dp_table[i][j] = with_current
                    total_cost_table[i][j] = total_cost_table[i - 1][j - current_item['cost']] + current_item['cost']
                else:
    ...
    return selected_items, dp_table[num_items][budget], total_cost_table[num_items][budget]
